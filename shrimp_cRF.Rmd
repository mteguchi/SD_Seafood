...
The variable importance metric is the mean difference in MSEs between OOB predictions and those from one variable permuted. The mean is computed from all trees and the mean divided by the standard deviation of the differences. 
...
The first one (SIZE_GROUP) is the same as the other method (cforest) but the second one (MARKETTYPE) was not the same. But the top three are still the same: SIZE_GROUP, MARKETTYPE, and SHRIMP_PROCESSING. 
