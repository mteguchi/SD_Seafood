...
Size and zip turned out to be pretty important.  The result is quite different when imporatance in randomForest is used...

```{r}
var.imp.df <- data.frame(importance(RF.M.1))
var.imp.df$variable <- rownames(var.imp.df) 
rownames(var.imp.df) <- NULL
#data.table::setorder(var.imp.df, -X.IncMSE)
...
rfPermute takes waaaaay too long to compute - so ignore it for now. 2019-02-25
