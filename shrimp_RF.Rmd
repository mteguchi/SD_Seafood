...
RF.M.1
```
About 67% of variability was explained by the forest, when PROD_METHOD was not included. 
...
```{r}
var.imp.RF.df <- data.frame(importance(RF.M.1)) 
var.imp.RF.df$Variable <- rownames(var.imp.RF.df)
rownames(var.imp.RF.df) <- NULL

# rearrange by increase in MSE in descending order
data.table::setorder(var.imp.RF.df, -X.IncMSE)

pred.RF.M.1 <- predict(RF.M.1)
  
var.imp.RF.df <- data.frame(var.imp.RF)
var.imp.RF.df$Variable <- rownames(var.imp.RF.df)
